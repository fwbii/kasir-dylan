...
// Proteksi halaman: Jika belum login atau session tidak lengkap
...
if(!isset($_SESSION['username']) || empty($_SESSION['username'])){
    header("location:../../auth/login.php?pesan=session_expired");
    exit();
}
...
<div class="d-flex">
    ...
    <div class="container-fluid p-4" style="margin-left: 0;">
        ...
        <div class="dashboard-header">
            <div class="d-flex justify-content-between align-items-center flex-wrap">
                <div>
                    <div class="welcome-text">Registrasi User</div>
        ...
        <?php if(isset($_GET['pesan'])): ?>
            <div class="alert alert-<?php echo $_GET['status'] ?? 'success'; ?> alert-dismissible fade show" role="alert" id="alertMessage">
                <i class="fas fa-<?php echo ($_GET['status'] ?? 'success') == 'success' ? 'check-circle' : 'exclamation-circle'; ?> me-2"></i>
                <?php 
                $pesan = $_GET['pesan'];
                if($pesan == 'sukses') {
                    $role_display = isset($_GET['role']) ? htmlspecialchars($_GET['role']) : 'Petugas';
                    echo "<strong>Sukses!</strong> " . $role_display . " berhasil ditambahkan!";
                }
                elseif($pesan == 'username_exists') echo "<strong>Username Sudah Ada!</strong> Username yang Anda masukkan sudah terdaftar dalam sistem. Silakan gunakan username lain.";
                elseif($pesan == 'field_kosong') echo "<strong>Data Tidak Lengkap!</strong> Semua field harus diisi. Silakan cek kembali form Anda.";
                elseif($pesan == 'role_invalid') echo "<strong>Role Tidak Valid!</strong> Pilih role yang sesuai: Admin atau Petugas Kasir.";
                elseif($pesan == 'db_error') echo "<strong>Gagal Menyimpan!</strong> Terjadi kesalahan saat menyimpan data. Silakan coba lagi.";
                elseif($pesan == 'gagal') echo "<strong>Gagal!</strong> Gagal menambahkan petugas!";
                elseif($pesan == 'hapus_sukses') echo "<strong>Sukses!</strong> Petugas berhasil dihapus!";
                elseif($pesan == 'hapus_gagal') echo "<strong>Gagal!</strong> Gagal menghapus petugas!";
                elseif($pesan == 'update_sukses') echo "<strong>Sukses!</strong> Petugas berhasil diupdate!";
                elseif($pesan == 'update_gagal') echo "<strong>Gagal!</strong> Gagal mengupdate petugas!";
                ?>
                <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
            </div>
            
            <!-- Script untuk menghapus query string dari URL -->
            <script>
                // Hapus parameter query dari URL setelah alert ditampilkan
            ...
            <div class="col-lg-5 mb-4">
                <div class="glass-card">
                    <div class="glass-header">
                        ...
                        <p class="text-muted small mb-0 mt-1">Isi form untuk menambahkan user baru</p>
                    </div>
                        ...
                        <div class="table-responsive">
                            <table class="table table-hover" id="tabelPetugas">
                                <thead class="table-dark">
                                    ...
                                    <tbody>
    <?php
    // Query yang benar - perhatikan huruf besar/kecil kolom
    $query = mysqli_query($conn, "SELECT * FROM user ORDER BY Role DESC, Username ASC");
    $no = 1;
    
    // Cek session sebelum menggunakan
    $current_user = isset($_SESSION['Username']) ? $_SESSION['Username'] : (isset($_SESSION['username']) ? $_SESSION['username'] : '');
    ...
    <tr>
        ...
        <td>
            <span class="role-badge <?= strtolower($role) == 'admin' ? 'role-admin' : 'role-petugas'; ?>">
                <?= ucfirst(strtolower($role)); ?>
            </span>
        </td>
        <td>
            <?php if(!empty($created_at) && $created_at != '0000-00-00'): ?>
        ...
        <td>
            <div class="btn-group btn-group-sm" role="group">
                <button type="button" class="btn btn-warning" onclick="editPetugas(<?= $id; ?>)">
                    <i class="fas fa-edit"></i>
                </button>
                <?php if($username != $current_user): ?>
                <button type="button" class="btn btn-danger" onclick="hapusPetugas(<?= $id; ?>, '<?= addslashes($username); ?>')">
    ...
    function simpanEdit() {
        ...
        .then(text => {
            try {
                const data = JSON.parse(text);
                if(data.success) {
                    // Gunakan pesan dari server yang sudah include role
                    showNotification('success', data.message || 'Data berhasil diupdate!');
                    setTimeout(() => {
                        location.reload();
                    }, 1500);
                } else {
                    showNotification('error', data.message || 'Gagal mengupdate data!');
                }
            } catch(e) {
                console.error('JSON Parse Error:', e);
                console.error('Response:', text);
...
function hapusPetugas(id, username) {
    console.log("ID yang akan dihapus:", id, "Username:", username);
    
    if(!id || id <= 0) {
        console.error("ID tidak valid:", id);
        alert('ID tidak valid! ID = ' + id);
        return;
    }
        // Tampilkan loading
        ...
        fetch('proses_hapus.php?id=' + encodeURIComponent(id), {
            method: 'GET',
            headers: {
        ...
        .then(data => {
            ...
            if(data.success) {
                ...
                showNotification('error', data.message || 'Gagal menghapus petugas!');
                
                // Jika error karena session, redirect ke login
...
function showNotification(type, message) {
    ...
    alertDiv.className = `alert alert-${type === 'success' ? 'success' : 'danger'} alert-dismissible fade show alert-notification position-fixed`;
